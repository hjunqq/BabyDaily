...
export class RecordRepository {
  ...
  async saveMany(entities: Partial<Record>[]): Promise<Record[]> {
    if (!entities.length) return [];
    const records = this.repo.create(entities);
  ...
  async getLatestEventTimes(babyId: string): Promise<{
    last_feed_time: Date | null;
    last_diaper_time: Date | null;
    ...
    const result = await this.repo
      .createQueryBuilder('r')
      .select([
        `MAX(CASE WHEN r.type = 'FEED' THEN r.time END) as last_feed_time`,
        `MAX(CASE WHEN r.type = 'DIAPER' THEN r.time END) as last_diaper_time`,
