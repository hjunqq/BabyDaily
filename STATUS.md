...
## ✅ 已完成
### 前端
- React + TypeScript + TailwindCSS 框架与主题组件基线，Dashboard/Mobile/OOTD 页面可用。
- 数据层接入真实后端接口：Dashboard/Mobile/OOTD 通过 `BabyService.ensureDevEnvironment` 自动登录 DEV 并创建默认家庭/宝宝后再取接口数据，不再使用本地 mock。
- 记录 CRUD：Dashboard 表格支持删除、编辑（RecordEditForm）、导出 CSV；表单校验基础逻辑完成。
- OOTD：列表按标签过滤、点赞、删除，图片上传（前端校验 MIME、限制 2MB、支持多图）。

### 后端
- NestJS 模块：auth（dev/微信登录）、users、family/baby 权限链路，FamilyGuard 校验 babyId 归属家庭。
- Record 模块：CRUD，summary `/records/baby/:id/summary`，trend `/records/baby/:id/trend`（snake_case 字段），CSV 导出 `/records/baby/:id/export`。
- OOTD 模块：列表（按月）、点赞、删除、上传（multer 本地存储，<=2MB，仅 image/*），自动填充 creator_id/date。
- 全局 ValidationPipe + AllExceptionsFilter/BusinessExceptionFilter；Swagger 基础配置；静态上传目录 `/uploads` 暴露。

---

## ⚠️ 当前状态
- Web 前端完全依赖真实接口，接口异常时直接显示错误态；无本地 fallback。
- 开发模式下自动登录并创建默认家庭/宝宝，正式环境需替换为真实登录与数据初始化。
- 前端代码与部分文档存在乱码，需统一为可读的 UTF-8 文本。
...
- [ ] 错误态与空态文案的 i18n 方案（含微信小程序）对齐设计规范。
